...
for train_index, test_index in kf.split(XX):
    ...
    local_dir = os.getcwd()
    config_path = os.path.join(local_dir, "config-feedforward-ecoli8")
    ...
    px_outputs = softmax(np.array(outputs).reshape(testsamplesize, 8), axis=1)
    # the index of maximum in each line
    pred_outputs = np.argmax(px_outputs, axis = 1)
    ...
    print("Train Accuracy:{}".format(winner.fitness))
    test_acc_list.append(acc)
    train_acc_list.append(winner.fitness)
    ...
    nodes_inputs = range(number_of_classification, number_of_classification+7)
    nodes_outputs = range(number_of_classification)
    nodes_mid = range(number_of_classification + 7, len(dict_nodes))
    ...
    cm = confusion_matrix(pred_outputs, real_outputs)
    print(cm)
    
    
print(test_acc_list,train_acc_list,node_list,connection_list)
print(np.mean(test_acc_list),np.mean(train_acc_list),np.mean(node_list),np.mean(connection_list))
