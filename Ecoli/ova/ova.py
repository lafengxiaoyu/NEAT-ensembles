...
for train_index, test_index in kf.split(XX):
    ...
    for j in range(testsamplesize):
        ...
        pred_array = np.array(pred).reshape(8,)
        #print(pred_array)
        if (np.sum(pred_array) >= 1):
            pred_value.append(np.where(pred_array==1))
        else:
            pred_value.append(np.array(range(8)))
    ...
    for j in range(samplesize):
        pred = []
        for i in range(8):
            [pred_outputs, real_outputs] = get_pred_real_train(i, j)
            #print(pred_outputs, real_outputs)
            pred.append(pred_outputs)
        #print(np.array(pred).T)
        pred_array = np.array(pred).reshape(8,)
    ...
    for i in range(samplesize):
        if (len(pred_value[i])== 1):
            #print(pred_  value[i][0], digits.target[1617+i])
            list_P.append(random.sample(pred_value[i][0].tolist(),1)[0])
        else:
            list_P.append(random.sample(pred_value[i].tolist(),1)[0])
