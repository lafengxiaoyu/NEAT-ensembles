...
for train_index, test_index in kf.split(XX):
    ...
    for j in range(num_test):
        ...
        for i in range(28):
            pred_outputs = get_pred_real_test(i, j)
    ...
    #print(list_P, y_test)
    print("Test accuracy:{}".format(np.sum(list_P == y_test)/num_test))
    ...
    print("Training accuracy: {}".format(np.sum(list_P == y)/num_train))
    winner_fitness = []
    for winner in winner_list:
        #print(winner.fitness)
        winner_fitness.append(winner.fitness)
    print("Average Training classifier accuracy: {}".format(np.mean(winner_fitness)))
